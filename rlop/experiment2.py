...
class Experiment2(TestCase):
    def _parameters(self):
        ...
        return r, mu, sigma, initial_price, strike_price, T, _dt, frictions, is_call_option, simplified, \
    ...
    def test_tc(self):
        r, mu, sigma, initial_price, strike_price, T, _dt, frictions, is_call_option, simplified, \
    ...
    def test_draw_plot(self):
        ...
        policies = dict()
        policies[0] = GaussianPolicy(simplified=simplified, alpha=1e-4,
                                     from_filename='trained_model/experiment1/simplified_mutation_lambda=1.00e-02/policy.pt')
        ...
        for j, friction in enumerate(all_frictions):
            for i, t in enumerate(times):
                axs[j].plot(initial_prices, nn_hedges[t][friction], label=f'{T - t:d}(NN)' if j == 0 else None,
                            c=palette[i], alpha=1 - 0.0 * j)
