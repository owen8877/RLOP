...
class GaussianPolicy(Policy):
    def __init__(self, simplified, alpha=1e-2, from_filename: str | None = None):
        super().__init__()
        if simplified:
    ...
    def update(self, delta, action, state, info):
        ...
        self.optimizer.step(loss_func)  # type: ignore
...
class GaussianPolicy_v2(Policy):
    def __init__(self, holder: NetHolder, alpha=1e-2, from_filename: str | None = None):
        super().__init__()
    ...
    def _gauss_param(self, tensor):
        tensor = tensor.float()
        mu = self.theta_mu_holder.net(tensor)[:, 1:2]
        sigma = self.theta_sigma(tensor)
...
class NNBaseline(Baseline):
    def __init__(self, simplified: bool, alpha=1e-2, from_filename: str | None = None):
        super().__init__()
        if simplified:
...
class NNBaseline_v2(Baseline):
    ...
    def _predict(self, tensor):
        y = self.holder.net(tensor.float())
        return y[:, 0]
    ...
    for e in range(episode_n):
        ...
        if V is not None:
            ...
            for t in range(T):
                ...
                if not pi_frozen:
                    pi.update(delta * np.power(env.gamma, t), actions[t], states[t], info)  # type: ignore
