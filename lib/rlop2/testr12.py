...
from .nn_v7 import GaussianPolicy_v7, NNBaseline_v7, PriceEstimator_v7
...
class Training(TestCase):
    def _parameters(self):
        ...
        return EnvSetup(
            ...
            r=0.04,
            mus=np.random.randn(parallel_n) * 1.0,
            sigmas=np.random.rand(parallel_n) * 1.0 + 0.1,
    ...
    def test_learn_price_first(self):
        ...
        bs_policy = BSPolicy(is_call=setup.is_call_option)
        nn_baseline = NNBaseline_v7(
            lr=1e-4,
            is_call_option=setup.is_call_option,
        ...
        nn_price_estimator = PriceEstimator_v7(
            nn_policy.theta_mu,
            nn_policy.optimizer,
        )
        ...
        policy_gradient(
            ...
            episode_n=200,
            pi_frozen=True,
            tensorboard_label=f"rlop2-{exp_name}-test-12",
        )
    ...
    def test_draw_plot(self):
        ...
        T, K, r, sigma, friction = 28 / 252, 1.0, 0.04, 0.6, 1e-3
        ...
        print(path)
        nn_policy = GaussianPolicy_v7(is_call_option=is_call_option, from_filename=f"{path}/policy_{check_point}.pt")
        nn_price_estimator = PriceEstimator_v7(nn_policy.theta_mu, nn_policy.optimizer)
    ...
    def test_invert_price(self):
        from lib.rlop2.nn_v7 import PriceEstimator_v7_grad_fn
        ...
        # load trained models
        path = self._path()
        nn_policy = GaussianPolicy_v7(is_call_option=is_call_option, from_filename=f"{path}/policy_1.pt")
        nn_price_estimator = PriceEstimator_v7_grad_fn(nn_policy.theta_mu, nn_policy.optimizer)
    ...
    def test_draw_influence_of_sigma(self):
        ...
        T, K, r, mu, friction = 56 / 252, 1, 0.04, 0, 1e-3
        ...
        for i, sigma in enumerate([0.6, 0.8, 1.0]):
            tensor = build_state_info_tensor(sigma)
            ...
            axs[0].plot(initial_spots, nn_price, label=f"sigma={sigma}", color=palette[i + 1], linestyle="-")
            axs[0].set_ylabel("Option Price")

            axs[1].plot(initial_spots, nn_latent_vol, label=f"sigma={sigma}", color=palette[i + 1], linestyle="-")
        ...
        path = "plot/rlop2/testr12/influence_of_sigma.png"
    ...
    def test_draw_influence_of_sigma_AI_conf(self):
        ...
        for i, sigma in enumerate([0.6, 0.8, 1.0]):
            ...
            nn_price = nn_price.cpu().numpy()
            nn_latent_vol = nn_latent_vol.cpu().numpy()
            axs[0].plot(initial_spots * 10, nn_price * 10, label=f"sigma={sigma}", color=palette[i + 1], linestyle="-")
            axs[0].set_ylabel("Option Price")

            axs[1].plot(initial_spots * 10, nn_latent_vol, label=f"sigma={sigma}", color=palette[i + 1], linestyle="-")
            axs[1].set_ylabel("Implied Vol")
        ...
        path = "plot/rlop2/testr12/influence_of_sigma_AI_conf.png"
        ensure_dir(path, need_strip_end=True)
        fig.savefig(path)
        fig.savefig(path.replace(".png", ".pdf"))
        # plt.show()
    ...
    def test_draw_influence_of_mu(self):
        ...
        T, K, r, sigma, friction = 28 / 252, 1.0, 0.04, 0.2, 1e-2
        ...
        path = "plot/rlop2/testr11/influence_of_mu.png"
        ensure_dir(path, need_strip_end=True)
        fig.savefig(path)
        plt.show()
    ...
    def test_draw_influence_of_friction(self):
        ...
        T, K, r, sigma, mu = 28 / 252, 1.0, 0.04, 0.2, 0
        initial_spots = np.linspace(0.8, 1.2, 50)
        ...
        path = "plot/rlop2/testr11/influence_of_friction.png"
        ensure_dir(path, need_strip_end=True)
        fig.savefig(path)
        plt.show()
