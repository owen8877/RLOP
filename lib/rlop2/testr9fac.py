...
class Training(TestCase):
    ...
    def learn_both_from_0_vol_latent_core(
        ...
        nn_baseline = NNBaseline_v6(
            lr=1e-4,
            is_call_option=setup.is_call_option,
            from_filename=f"{path}/baseline_{check_point}.pt" if check_point else None,
            groups=network_depth,
        )
        ...
        env = RLOPEnv(replace(setup, risk_lambdas=np.array([risk_lambda] * N_parallel)), nn_price_estimator)
        ...
        collector = policy_gradient(
            ...
            nn_price_estimator,
            episode_n=episode_n,
            tensorboard_label=f"rlop2-{exp_name}-{risk_lambda=}-{network_depth=}",
        )
        ...
        nn_baseline.save(f"{path}/baseline_{check_point_save}_R{replica}.pt")
    ...
    def test_invert_price_set1(self):
        ...
        ensure_dir(dir := "result/rlop/testr9fac")
        ...
        print(df.pivot_table(index=["risk_lambda"], columns="network_depth", values=["time_used"], aggfunc="mean"))

    def test_analyze_set1(self):
        df = pd.read_csv("result/rlop/testr9fac/set1.csv")
        output_dir = "result/rlop/testr9fac/set1"
