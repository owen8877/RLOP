...
from .nn_v3 import NNBaseline_v3
...
class Training(TestCase):
    ...
    def test_separate_net(self):
        ...
        for risk_lambda in risk_lambdas:
            ...
            policy_gradient(
                ...
                nn_baseline,
                episode_n=200,
                tensorboard_label=f"qlbs2-{exp_name}-separate-net",
                pi_frozen=True,
            )
    ...
    def test_draw_plot(self):
        ...
        initial_spots = np.linspace(0.6, 1.4, 50)

        _, axs = plt.subplots(2, 1, figsize=(7, 6), constrained_layout=True, sharex=True)
        ...
        nn_baseline = NNBaseline_v3(lr=1e-4, from_filename=f"{path}/baseline_{check_point}.pt")
        ...
        for i, mu in enumerate([-0.2, 0.0, 0.2]):
            ...
            if i == 0:
                ...
                # axs[1].plot(initial_spots, bs_price, label="BS Policy", color=palette[0], linestyle="--")
