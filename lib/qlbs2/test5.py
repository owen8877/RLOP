...
from .nn_v4 import GaussianPolicy_v4, NNBaseline_v4
...
class Training(TestCase):
    ...
    def test_learn_both_from_0_vol_latent(self):
        ...
        for risk_lambda in risk_lambdas:
            ...
            policy_gradient(
                ...
                nn_baseline,
                episode_n=200,
                tensorboard_label=f"qlbs2-{exp_name}-learn-both-from-0-vol-latent",
            )
    ...
    def test_draw_plot(self):
        ...
        print(path)
        nn_policy = GaussianPolicy_v4(is_call_option=is_call_option, from_filename=f"{path}/policy_{check_point}.pt")
        nn_baseline = NNBaseline_v4(from_filename=f"{path}/baseline_{check_point}.pt")
