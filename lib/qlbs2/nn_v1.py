...
class GaussianPolicy_v1(Policy):
    ...
    def batch_action(self, state_info_tensor, random: bool = True):
        ...
        with torch.no_grad():
            ...
            if random:
                _mu = torch.special.logit(mu, 1e-6)
    ...
    def save(self, filename: str):
        lib.util.ensure_dir(filename, need_strip_end=True)
        torch.save({"sigma_net": self.theta_sigma.state_dict(), "optimizer": self.optimizer.state_dict()}, filename)
...
class NNBaseline_v1(Baseline):
    ...
    def update(self, G: np.ndarray, state: State, info: Info):
        ...
        def loss_func():
            _G = torch.log(torch.clamp(-G, min=1e-6))  # type: ignore
    ...
    def save(self, filename: str):
        lib.util.ensure_dir(filename, need_strip_end=True)
        torch.save({"optimizer": self.optimizer.state_dict()}, filename)
