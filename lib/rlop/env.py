...
import lib.util
from .interface import InitialEstimator
        ...
        return [
            torch.tensor(
                [
                    ...
                    lib.util.standard_to_normalized_price(info.strike_price, info.mu, info.sigma, t + 1, info._dt),
                    info.r,
                    info.mu,
                    info.sigma,
                    info.friction,
            ...
            for t in range(self.remaining_step)
        ]
...
class RLOPEnv(gym.Env):
    def __init__(
        ...
        r: float,
        friction: float,
        initial_estimator: InitialEstimator,
        initial_asset_price: float,
        *,
        mutation: Union[float, Callable] = 0.01,
        _dt: float = 1,
    ):
    ...
    def step(self, action):
        ...
        payoff = lib.util.payoff_of_option(self.is_call_option, S_t, self.info.strike_price)
    ...
    def render(self, mode="human", axs=None):
        ...
        for i in range(T):
            if i < tau:
                payoff = lib.util.payoff_of_option(
                    self.is_call_option,
                    self._standard_price[i + 1],
                    self.info.strike_price,
                )
                ax_option.plot(
                    np.arange(0, i + 2),
                    self.portfolio_value_history[0 : i + 2, i],
                    label=f"#{i:d}",
                    c=palette[i],
                )
                ...
                ax_option.plot(
                    np.arange(0, tau + 1),
                    self.portfolio_value_history[0 : tau + 1, i],
                    label=f"#{i:d}",
                    c=palette[i],
                )
...
class Test(TestCase):
    def test_step(self):
        ...
        env = RLOPEnv(
            ...
            r=0.025e-2,
            friction=0,
            initial_estimator=BSInitialEstimator(True),
