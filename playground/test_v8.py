...
# ============================================================
# Realized-path helpers (历史 world)
# ============================================================

def build_spot_series_from_options_df(df_pre: pd.DataFrame, symbol: str) -> pd.Series:
    """
    Build a daily SPOT series from the options dataframe (which has repeated S per day).
    Returns pd.Series indexed by date (Timestamp normalized), values = median S for that day.
    """
    d = df_pre.copy()
    d["date"] = pd.to_datetime(d["date"]).dt.normalize()
    d = d[d["act_symbol"] == symbol].copy()
    if "S" not in d.columns:
        raise ValueError("Input df_pre must contain column 'S' to hedge on realized paths.")
...
def make_realized_path(
    ...
    """
    Extract realized path S(t) from start to end (inclusive) using available trading days.
    If require_end=True, we require the spot_series to have an observation exactly on 'end'.
    Returns (dates: np.ndarray[Timestamp], S_path: np.ndarray[float]) or (None, None) if missing.
    """
    s = spot_series.loc[(spot_series.index >= start) & (spot_series.index <= end)]
    if len(s) < 2:
        return None, None
    if require_end and (s.index[-1] != end):
        return None, None
    return s.index.to_numpy(), s.to_numpy(dtype=float)


def precompute_delta_grid_tau_list(
    price_scalar_fn: Callable[[float, float, float, float, float], float],
    K: float,
    r: float,
    q: float,
    ...
    """
    Like precompute_delta_grid but uses an explicit tau_list for each rebalance step.
    tau_list length = n_reb (rebalance times), excludes maturity.
    Returns delta_grid shape (n_reb, grid_n).
    """
    Sg = np.asarray(S_grid, dtype=float)
    tau_list = np.asarray(tau_list, dtype=float)
    n_reb = int(tau_list.size)
    grid_n = int(Sg.size)

    prices = np.empty((n_reb, grid_n), dtype=float)
    for t in range(n_reb):
        tau = float(max(tau_list[t], 0.0))
        prices[t, :] = np.array(
...
def hedge_short_call_realized_path(
    dates: np.ndarray,
    S_path: np.ndarray,
    K: float,
    r: float,
    premium0: float,
    friction: float,
    get_delta: Callable[[int, float], float],
) -> Dict[str, float]:
    """
    Delta hedge a SHORT call on ONE realized spot path.
    Rebalance at each date except the last (maturity date).
    Cash accrues with exp(r * dt) where dt uses actual calendar gaps / 365.

    Returns per-hedge scalars:
      error_T, cost_total
    """
    dates = np.asarray(dates)
    S_path = np.asarray(S_path, dtype=float)
    if S_path.ndim != 1 or len(S_path) < 2:
        raise ValueError("S_path must be 1D with length >= 2")

    n_nodes = len(S_path)          # includes maturity node
    n_reb = n_nodes - 1            # number of rebalance times (exclude maturity)
    S0 = float(S_path[0])

    ST = float(S_path[-1])
    payoff = max(ST - float(K), 0.0)

    # t=0: receive premium, buy delta shares
    delta = float(get_delta(0, S0))
    trade0 = delta
    cost_total = float(friction) * abs(trade0) * S0 if friction > 0.0 else 0.0
    cash = float(premium0) - trade0 * S0 - cost_total

    # Rebalance at nodes 1..n_reb-1
    for i in range(1, n_reb):
        # accrue cash from dates[i-1] -> dates[i]
        dt = (pd.Timestamp(dates[i]) - pd.Timestamp(dates[i - 1])).days / 365.0
        cash *= math.exp(float(r) * float(max(dt, 0.0)))

        Si = float(S_path[i])
        new_delta = float(get_delta(i, Si))
        trade = new_delta - delta

        if friction > 0.0:
            c = float(friction) * abs(trade) * Si
            cost_total += c
            cash -= c
    ...
    # accrue cash last interval (last rebalance node -> maturity node)
    dt_last = (pd.Timestamp(dates[-1]) - pd.Timestamp(dates[-2])).days / 365.0
    cash *= math.exp(float(r) * float(max(dt_last, 0.0)))
...
class HedgingRunConfig:
    ...
    n_rep_days: int = 5
    rep_day_mode: str = "even"  # "even", "middle", or "all"
    ...
    n_steps: int = 28                 # kept for completeness; not used in realized tau_list
    n_paths: int = 2000               # unused in 历史 world
    friction: float = 4e-3
    seed: int = 123                   # unused in 历史 world
    ...
    # Realized-path controls
    use_realized_paths: bool = True
    require_expiry_spot: bool = True
...
def run_dynamic_hedging_spy_like(
    ...
    One run producing long-form rows:
      (date, bucket, moneyness_target, model) -> realized-path hedge scalars

    历史 world:
      - Underlying path is the realized spot series from df_all.
      - For each (rep_day, bucket), we hedge *one* realized path from rep_day to anchor_exp.
      - Each row contains (error_T, cost_total); publication tables recompute RMSE / shortfall from these.
    """
    if not cfg.use_realized_paths:
        raise NotImplementedError("This version is for 历史-world hedging only (use_realized_paths=True).")

    # Preprocess to expected schema
    df_pre = adapter_eur_calls_to_summarizer(df_all)

    # Build daily SPOT series for realized hedging (uses all dates available in df_all for this symbol)
    spot_series = build_spot_series_from_options_df(df_pre, cfg.symbol)

    # Filter option chains for selecting rep_days / calibrating
    ...
    for day_i, rep_day in enumerate(rep_days):
        ...
        for b_center in bucket_centers:
            ...
            anchor_exp = pd.Timestamp(anchor_row["expiration"]).normalize()
            ...
            # Selection: strikes for targets (within anchor expiry)
            ...
            # World sigma_true (only used for S_grid width and diagnostics)
            ...
            # --------------------------------------------------------
            # REALIZED PATH (rep_day -> anchor_exp)
            # --------------------------------------------------------
            dates_path, S_path = make_realized_path(
                spot_series=spot_series,
                start=pd.Timestamp(rep_day).normalize(),
                end=anchor_exp,
                require_end=bool(cfg.require_expiry_spot),
            )
            if dates_path is None:
                continue

            # tau_list at rebalance times (exclude maturity node)
            tau_list = np.array(
                [(anchor_exp - pd.Timestamp(dates_path[i])).days / 365.0 for i in range(len(dates_path) - 1)],
                dtype=float,
            )
            if len(tau_list) < 1:
                continue

            # Use the realized S0 at rep_day for grid and diagnostics
            S0_realized = float(S_path[0])
            T0 = float(tau_list[0])
            ...
            # --------------------------------------------------------
            # Precompute delta grids per option & model (variable tau_list)
            # --------------------------------------------------------
            S_grid = build_S_grid(
                S0=S0_realized,
                sigma_true=sigma_true,
                T=float(max(T0, 1e-12)),
                grid_n=cfg.grid_n,
                n_std=cfg.grid_n_std,
            )
            ...
            for opt_idx in range(len(selected)):
                ...
                for model in price_fns.keys():
                    if model == "BS":
                        continue
                    dg = precompute_delta_grid_tau_list(
            ...
            # --------------------------------------------------------
            # Hedge each selected strike along realized path
            # --------------------------------------------------------
            ...
            for opt_idx in range(len(selected)):
                ...
                # Diagnostics: "world-consistent" BS premium at t=0 using sigma_true
                premium0_world_bs = bs_call_from_spot(S0_realized, K, T0, r, q, sigma_true)
                ...
                for model in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    if model == "BS":
                        def get_delta(step_idx: int, S_val: float, _K=K, _r=r, _q=q, _sigma_bs=sigma_bs) -> float:
                            tau_t = float(max(tau_list[int(step_idx)], 0.0))
                            return float(bs_delta_from_spot(np.array([float(S_val)]), _K, tau_t, _r, _q, float(_sigma_bs))[0])
                    else:
                        dg = delta_grids[(model, opt_idx)]
                        def get_delta(step_idx: int, S_val: float, _dg=dg, _S_grid=S_grid) -> float:
                            return float(interp_delta(_dg, _S_grid, int(step_idx), np.array([float(S_val)]))[0])

                    metrics = hedge_short_call_realized_path(
                        dates=dates_path,
                        S_path=S_path,
                        K=K,
                        r=r,
                        premium0=premium0_mkt,      # FAIRNESS: market premium across models
                        friction=float(cfg.friction),
                        get_delta=get_delta,
                    )
                    ...
                    row = {
                        ...
                        "T_years": float(T0),
                        "T_days_365": float(T0) * 365.0,
                        "S0_realized": float(S0_realized),
                        ...
                        "model": model,
                        "path_len": int(len(S_path)),
                    }
                    row.update(metrics)  # adds error_T, cost_total
                    results.append(row)

            # Save per (day,bucket) artifacts (optional)
            if outp is not None:
                ...
                # Save realized path
                pd.DataFrame({"date": dates_path, "S": S_path}).to_csv(subdir / "realized_path.csv", index=False)

                # Save delta grids
                dz = {}
                for (model, opt_idx), dg in delta_grids.items():
                    dz[f"delta_{model}_opt{opt_idx}"] = dg.astype(np.float32)
                np.savez_compressed(subdir / "delta_grids.npz", S_grid=S_grid.astype(np.float32), tau_list=tau_list.astype(np.float32), **dz)

    if not results:
        raise RuntimeError("No hedging results produced. Check data coverage to expiration for realized paths.")
...
def make_hedging_publication_table(
    ...
    """
    Works for realized-path hedging where hedge_res contains:
      - error_T (terminal hedging error = V_T - payoff)
      - cost_total (total transaction costs)
    ...
    def _agg_metric(sub: pd.DataFrame) -> float:
        if sub.empty:
            return float("nan")
        if metric == "avg_cost":
            return float(sub["cost_total"].mean())
        ...
        if metric == "shortfall_1pct":
            # 1% lower-quantile of hedging error
    ...
    for sec_name, sec_key in sections:
        for b in bucket_labels:
            ...
            for m in models:
                row[m] = _agg_metric(sub[sub["model"] == m])
            rows.append(row)
...
def hedging_spy20():
    ...
    cfg = HedgingRunConfig(
        ...
        run_rlop=True,
        # Saving
        out_dir="SPY_20Q1_hedging_v_hist",
        run_tag=None,
        use_realized_paths=True,
        require_expiry_spot=True,
    )

    hedge_res = run_dynamic_hedging_spy_like(df_all=df, cfg=cfg)

    print("\nDynamic hedging summary (SPY 20Q1, 历史 world):")

    def _rmse(x: pd.Series) -> float:
        e = x.to_numpy(dtype=float)
        return float(np.sqrt(np.mean(e * e)))

    def _shortfall(x: pd.Series) -> float:
        e = x.to_numpy(dtype=float)
    ...
    summary = (
        ...
        .agg(
            RMSE_hedge=("error_T", _rmse),
            cost_mean=("cost_total", "mean"),
            shortfall_prob=("error_T", _shortfall),
            N=("error_T", "size"),
        )
        .sort_values(["bucket", "moneyness_target", "RMSE_hedge"])
    )
...
def hedging_xop20():
    ...
    cfg = HedgingRunConfig(
        ...
        run_rlop=True,
        # Saving
        out_dir="XOP_20Q1_hedging_v_hist",
        run_tag=None,
        use_realized_paths=True,
        require_expiry_spot=True,
    )
    ...
    print("\nDynamic hedging summary (XOP 20Q1, 历史 world):")
