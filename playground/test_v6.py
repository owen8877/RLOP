...
def make_realized_path(spot_series: pd.Series, start: pd.Timestamp, end: pd.Timestamp, require_end: bool = True):
    """
    ...
    Returns (dates: np.ndarray[Timestamp], S_path: np.ndarray[float]).
    """
...
def hedge_short_call_realized_path(
    ...
    # Payoff uses terminal spot at last node
    ST = float(S_path[-1])
...
def run_dynamic_hedging_spy_like(
    ...
    Key change:
      If cfg.use_realized_paths=True, we hedge on the REAL SPOT path (single path),
      and compute tail metrics later by aggregating error_T across hedges.
    """
    # Preprocess to expected schema
    df_pre = adapter_eur_calls_to_summarizer(df_all)

    # Build daily SPOT series for realized hedging (uses all dates available in df_all)
    spot_series = build_spot_series_from_options_df(df_pre, cfg.symbol) if cfg.use_realized_paths else None
    ...
    for day_i, rep_day in enumerate(rep_days):
        ...
        for b_center in bucket_centers:
            ...
            # World sigma_true (still used for delta-grid width)
            ...
            # --------------------------------------------------------
            if cfg.use_realized_paths:
                if spot_series is None:
                    raise RuntimeError("spot_series not built but use_realized_paths=True")
                ...
                # (keeps your old simulated-path behavior)
                T = float(anchor_row["tau"])
                seed_db = int(cfg.seed + day_i * 10007 + b_center * 101)
                S_paths = simulate_spot_paths(
                    S0=S0, r=r, q=q, sigma_true=sigma_true,
                    T=T, n_steps=cfg.n_steps, n_paths=cfg.n_paths, seed=seed_db
                )
                # not used in the realized-path branch below
                raise NotImplementedError("Set use_realized_paths=True for this experiment.")
            ...
            for opt_idx in range(len(selected)):
                ...
                T0 = float(tau_list[0])
                premium0_world_bs = bs_call_from_spot(float(S_path[0]), K, T0, r, q, sigma_true)
                ...
                for model in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    if model == "BS":
                        def get_delta(step_idx: int, S_val: float) -> float:
                            tau = float(max(tau_list[int(step_idx)], 0.0))
                            return float(bs_delta_from_spot(np.array([float(S_val)]), K, tau, r, q, float(sigma_bs))[0])
                    else:
                        dg = delta_grids[(model, opt_idx)]
                        def get_delta(step_idx: int, S_val: float) -> float:
                            return float(interp_delta(dg, S_grid, int(step_idx), np.array([float(S_val)]))[0])
