...
def prepare_calls_one_day_symbol(
    ...
    """
    type='american' : Fast-path. Use provided F/DF/(r) and (ideally) spot S and PV_div.
                      Uses C_eur as market mid if present.
                      Computes an implied continuous dividend yield q_hat per expiry for step-2 hedging.
    type='european' : Calls & Puts pairing + parity inference (original behavior).
    ...
    # =========================================================================
    # FAST-PATH for 'american': Calls-only, trust supplied F/DF/(r), and keep spot info if available
    # =========================================================================
    if type.lower() == "american":
        ...
        hdr = df.groupby(["date", "symbol", "expiration"], as_index=False).agg(
            F=("F", "first"),
            DF=("DF", "first"),
            S=("S", "first") if "S" in df.columns else ("F", "first"),
            PV_div=("PV_div", "first"),
        )
        ...
        # r: prefer provided, else infer from DF
        ...
        # fill/repair spot if missing: S ≈ F*DF + PV_div (from your conversion logic)
        S_infer = hdr["F"].to_numpy() * hdr["DF"].to_numpy() + hdr["PV_div"].to_numpy()
        hdr["S"] = np.where(np.isfinite(hdr["S"].to_numpy()), hdr["S"].to_numpy(), S_infer)

        # implied continuous dividend yield q_hat: F = S * exp((r-q)tau)
        ...
        # calls table shaped like downstream expects
        calls = df.groupby(["date", "symbol", "expiration", "tau", "strike"], as_index=False).agg(C_mid=("mid", "first"))
        calls = calls.merge(
            hdr[["date", "symbol", "expiration", "F", "DF", "r", "S", "PV_div", "q_hat"]],
        ...
        calls["sigma_mkt_b76"] = calls.apply(
            lambda rr: b76_iv_from_price(rr["C_mid"], rr["F"], rr["strike"], rr["tau"], rr["r"]), axis=1
        )
        ...
        # parity_df: header + coverage proxy (#strikes)
...
def adapter_eur_calls_to_summarizer(calls_out: pd.DataFrame) -> pd.DataFrame:
    ...
    Input (from preprocess_american_to_european):
      required cols:
        date, expiration, strike, C_eur, F, DF
      plus (recommended for step-2 correctness):
        S, r, PV_div
      and either act_symbol OR symbol

    Output for summarizer / hedging:
      date, act_symbol, expiration, strike, cp="Call", bid, ask, mid,
      S, r, PV_div, F, DF
    """
    df = calls_out.copy()
    ...
    if "act_symbol" in df.columns:
        ...
        raise ValueError("calls_out must contain 'act_symbol' or 'symbol'.")
    ...
    # Optional but important for spot/forward consistency
    if "S" in df.columns:
        df["S"] = pd.to_numeric(df["S"], errors="coerce")
    else:
        df["S"] = np.nan
    ...
    # Bid/ask = mid (keeps filters happy)
    out["bid"] = out["mid"]
    out["ask"] = out["mid"]

    out = out.dropna(subset=["date", "act_symbol", "expiration", "strike", "mid", "F", "DF"])
    out = out[(out["mid"] > 0) & (out["ask"] >= out["bid"]) & (out["DF"] > 0)]

    out = out[
        ["date", "act_symbol", "expiration", "strike", "cp", "bid", "ask", "mid", "S", "r", "PV_div", "F", "DF"]
    ].sort_values(["date", "act_symbol", "expiration", "strike"]).reset_index(drop=True)
...
def _simulate_gbm_paths(
    ...
    """
    Spot GBM under risk-neutral dynamics with continuous dividend yield q:
...
def _hedge_paths(
    ...
    delta_fn,
    friction: float = 0.0,
    premium0: float | np.ndarray | None = None,
) -> Dict[str, float]:
    """
    Delta-hedge a short call using given delta_fn, starting from a FIXED initial premium.

    Key fairness fix:
      - premium0 is the SAME across models (typically market mid at t=0).
      - price_fn is used only to compute deltas (via delta_fn), not to set initial cash.

    Timing convention:
      - At t=0: receive premium0, trade to delta0 at S0 (pay friction).
      - For each step: cash accrues at r over dt, then rebalance at St (pay friction).
      - At maturity: portfolio value minus payoff is the hedging error.
    ...
    # Shared initial premium (market) across models
    if premium0 is None:
        raise ValueError("premium0 must be provided for fair hedging comparisons.")
    if np.isscalar(premium0):
        prem = np.full_like(S0, float(premium0), dtype=float)
    else:
        prem = np.asarray(premium0, dtype=float)
        if prem.shape != S0.shape:
            raise ValueError(f"premium0 shape {prem.shape} must match S0 shape {S0.shape}.")

    # Initial delta and initial trade (from 0 shares)
    tau0 = T
...
def run_dynamic_hedging_for_symbol_period(
    ...
    # ---- Step-B additions ----
    n_rep_days: int = 5,
    rep_day_mode: str = "even",  # "even" (evenly spaced) or "middle" (old behavior)
    moneyness_targets: Optional[List[float]] = None,  # e.g. [0.9,0.97,1.0,1.03,1.1]
    calibrate_on: str = "expiry",  # "expiry" (anchor expiry only, faster) or "bucket" (full bucket)
    sigma_true_mode: str = "bucket_median",  # "bucket_median" or "expiry_median"
    # RL knobs (fix mismatch by default)
    ...
    """
    Step-B runner: multi-day × multi-moneyness dynamic hedging.

    For each representative day:
      - Build calls surface
      - For each bucket:
          * choose an anchor expiry closest to bucket center (so T fixed)
          * select multiple strikes by moneyness targets within that expiry
          * calibrate models (on anchor expiry or full bucket)
          * simulate ONE GBM path set for (day,bucket,expiry)
          * hedge each selected strike using SAME S_paths, fair initial premium0 = market C_mid
    Returns long-form DataFrame with one row per (day,bucket,m_target,model).
    """
    if moneyness_targets is None:
        moneyness_targets = [0.90, 0.97, 1.00, 1.03, 1.10]
    moneyness_targets = [float(x) for x in moneyness_targets]

    if calibrate_on not in ("expiry", "bucket"):
        raise ValueError("calibrate_on must be 'expiry' or 'bucket'.")
    if sigma_true_mode not in ("bucket_median", "expiry_median"):
        raise ValueError("sigma_true_mode must be 'bucket_median' or 'expiry_median'.")
    if rep_day_mode not in ("even", "middle"):
        raise ValueError("rep_day_mode must be 'even' or 'middle'.")

    # --- Preprocess calls, consistent with your IVRMSE entry logic ---
    if type == "american":
        df_pre = adapter_eur_calls_to_summarizer(df_all)
    else:
        df_pre = preprocess_deribit(df_all)
    ...
    rep_days = _pick_rep_days(unique_days, n_rep_days, rep_day_mode)

    # --- bucket mapper using requested centers ---
    bucket_centers = [int(x) for x in buckets]
    bucket_labels = [f"{d}d" for d in bucket_centers]

    def _assign_bucket_label(tau_years: float) -> str:
        days = float(tau_years) * 365.0
        i = int(np.argmin([abs(days - c) for c in bucket_centers]))
        return f"{bucket_centers[i]}d"

    period_str = f"{start_date}..{end_date}"
    results_rows: List[Dict[str, object]] = []

    # Import RL classes once (lazy import if not used)
    QLBSModel = None
    RLOPModel = None
    if run_qlbs:
    ...
    for day_i, rep_day in enumerate(rep_days):
        ...
        calls["bucket"] = calls["tau"].apply(_assign_bucket_label)

        for d_label, d_center in zip(bucket_labels, bucket_centers):
            calls_b = calls[calls["bucket"] == d_label].copy()
            if calls_b.empty:
                continue

            # Choose anchor expiry: nearest tau_days to bucket center
            calls_b["tau_days"] = calls_b["tau"] * 365.0
            idx_anchor = (calls_b["tau_days"] - float(d_center)).abs().idxmin()
            ...
            # Anchor contract settings (shared across strikes in this expiry)
            F0 = float(anchor_row["F"])
            r = float(anchor_row["r"])
            T = float(anchor_row["tau"])
            S0 = F0  # keep your convention for now

            anchor_slice["moneyness_F"] = anchor_slice["strike"] / anchor_slice["F"]

            # Select one option per target moneyness within anchor expiry
            selected_opts = []
            for m_target in moneyness_targets:
                j = (anchor_slice["moneyness_F"] - m_target).abs().idxmin()
                selected_opts.append(anchor_slice.loc[j])

            # Calibration dataset (expiry-only or full bucket)
            cal_df = anchor_slice if calibrate_on == "expiry" else calls_b

            # World sigma for GBM simulation (unchanged Step B)
            if sigma_true_mode == "expiry_median":
                sigma_true = float(anchor_slice["sigma_mkt_b76"].median())
            else:
                sigma_true = float(calls_b["sigma_mkt_b76"].median())

            # --------------------------------------------------------
            # Calibrate models once per (day, bucket, anchor expiry)
            # --------------------------------------------------------
            sigma_bs = fit_sigma_bucket(cal_df) if run_bs else np.nan
            ...
            h_params = None
            if run_heston:
                h_params, _ = calibrate_heston_bucket(cal_df, u_max=100.0, n_points=501)

            Qmodel, q_result = None, None
            if run_qlbs:
                Qmodel = QLBSModel(
                    is_call_option=True,
                    checkpoint=qlbs_checkpoint_tmpl.format(rl=qlbs_risk_lambda),
                    anchor_T=28 / 252,
                )
                time_to_expiries = cal_df["tau"].to_numpy()
                strikes = cal_df["strike"].to_numpy()
                observed_prices = cal_df["C_mid"].to_numpy()
            ...
            Rmodel, r_result = None, None
            if run_rlop:
                Rmodel = RLOPModel(
                    is_call_option=True,
                    checkpoint=rlop_checkpoint,
                    anchor_T=28 / 252,
                )
                time_to_expiries = cal_df["tau"].to_numpy()
                strikes = cal_df["strike"].to_numpy()
            ...
            # --------------------------------------------------------
            # Build model price/delta functions (vector S)
            # --------------------------------------------------------
            price_fns = {}
            delta_fns = {}
            ...
            if run_bs and np.isfinite(sigma_bs):
                def bs_price_fn(S_vec, K_, tau_, r_):
                    ...
                    for i, Si in enumerate(S_vec):
                        out[i] = b76_price_call(float(Si), float(K_), float(tau_), float(r_), float(sigma_bs))
                    return out
            ...
            if run_jd and jd_params:
                sigma_jd = float(jd_params["sigma"])
                lam_jd = float(jd_params["lam"])
                muJ_jd = float(jd_params["muJ"])
                dJ_jd = float(jd_params["deltaJ"])

                def jd_price_fn(S_vec, K_, tau_, r_):
                    S_vec = np.atleast_1d(S_vec)
                    out = np.empty_like(S_vec, dtype=float)
                    for i, Si in enumerate(S_vec):
                        out[i] = merton_price_call_b76(float(Si), float(K_), float(tau_), float(r_),
                                                       sigma_jd, lam_jd, muJ_jd, dJ_jd)
                    return out
            ...
            if run_heston and h_params:
                def sv_price_fn(S_vec, K_, tau_, r_):
                    ...
                    out = np.empty_like(S_vec, dtype=float)
                    for i, Si in enumerate(S_vec):
                        out[i] = heston_price_call(float(Si), float(K_), float(tau_), float(r_),
                                                   h_params, u_max=60.0, n_points=201)
                    return out
                price_fns["SV"] = sv_price_fn
                delta_fns["SV"] = lambda S_vec, K_, tau_, r_: _delta_from_pricer(sv_price_fn, S_vec, K_, tau_, r_)

            if run_qlbs and q_result is not None:
                sigma_q, mu_q = float(q_result.sigma), float(q_result.mu)
                ...
                def qlbs_price_fn(S_vec, K_, tau_, r_):
                    ...
                    for i, Si in enumerate(S_vec):
                        ...
                        out[i] = float(res.estimated_prices[0])
                    return out
                price_fns["QLBS"] = qlbs_price_fn
                delta_fns["QLBS"] = lambda S_vec, K_, tau_, r_: _delta_from_pricer(qlbs_price_fn, S_vec, K_, tau_, r_)

            if run_rlop and r_result is not None:
                sigma_r, mu_r = float(r_result.sigma), float(r_result.mu)
            ...
            # --------------------------------------------------------
            # Simulate ONE set of GBM paths for this (day,bucket,expiry)
            # --------------------------------------------------------
            # Use a deterministic but unique seed per (day,bucket) so replication is stable
            seed_db = int(seed + day_i * 10007 + d_center * 101)
            ...
            # --------------------------------------------------------
            # Hedge each selected strike (moneyness target) under same paths
            # --------------------------------------------------------
            for opt_j, opt in enumerate(selected_opts):
                K = float(opt["strike"])
                premium0 = float(opt["C_mid"])
                m_actual = float(opt["strike"] / opt["F"])
                exp_used = opt["expiration"]
                ...
                for model_name in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    row = {
                        ...
                        "period": period_str,
                        "bucket": d_label,
                        "anchor_expiration": exp_used,
                        "T_days": T * 252.0,
                        "sigma_true": sigma_true,
                        "S0": S0,
                        "K": K,
                        "premium0": premium0,
                        "moneyness_target": float(moneyness_targets[opt_j]),
                        "moneyness_actual": m_actual,
                        "model": model_name,
                    }
                    row.update(metrics)
    ...
    if not results_rows:
        raise RuntimeError("run_dynamic_hedging_for_symbol_period: no results produced (check filters).")
