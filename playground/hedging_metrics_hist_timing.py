...
def _finite_timing_summary(values: List[float]) -> Dict[str, float]:
    ...
    return {
        ...
        "p10_s": float(np.quantile(arr, 0.10)),
...
def _summarize_timing_rows(df: pd.DataFrame, value_col: str, group_cols: List[str]) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame(columns=group_cols + ["n", "mean_s", "std_s", "p10_s", "p50_s", "p90_s", "min_s", "max_s"])
    rows: List[Dict[str, Any]] = []
    for keys, sub in df.groupby(group_cols, dropna=False):
        if not isinstance(keys, tuple):
            keys = (keys,)
        row = {k: v for k, v in zip(group_cols, keys)}
        row.update(_finite_timing_summary(sub[value_col].astype(float).tolist()))
        rows.append(row)
    return pd.DataFrame(rows)


@dataclass
class TimingRunConfig:
    timing_max_paths_per_model: int = 50
    timing_warmup_paths_per_model: int = 5
    save_path_rows: bool = True
    save_fit_rows: bool = True
    save_grid_rows: bool = True



def benchmark_efficiency_spy_like(
    df_all: pd.DataFrame,
    cfg: HedgingRunConfig,
    timing_cfg: TimingRunConfig,
) -> Dict[str, pd.DataFrame]:
    """
    Measures two quantities using the SAME market-data / hedging code path as the paper:
      1) per-slice calibration / RL fit time
      2) per-hedging-path latency (one option on one realized path)

    Path latency is timed around hedge_short_call_paths only, excluding CSV writing.
    Warmup paths are collected but excluded from the main summary.
    """
    df_pre = adapter_eur_calls_to_summarizer(df_all)
    df_pre["date"] = pd.to_datetime(df_pre["date"]).dt.normalize()
    mask = (
    ...
    if cfg.out_dir is not None:
        outp = Path(cfg.out_dir)
        outp.mkdir(parents=True, exist_ok=True)
        run_tag = cfg.run_tag or f"timing_{cfg.symbol}_{cfg.start_date}_{cfg.end_date}_{cfg.world}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        outp = outp / run_tag
        outp.mkdir(parents=True, exist_ok=True)
        _json_dump(outp / "timing_config.json", {**cfg.__dict__, **timing_cfg.__dict__})
    ...
    active_models = []
    if cfg.run_bs:
        active_models.append("BS")
    if cfg.run_jd:
        active_models.append("JD")
    if cfg.run_heston:
        active_models.append("SV")
    if cfg.run_qlbs:
        active_models.append("QLBS")
    if cfg.run_rlop:
        active_models.append("RLOP")

    path_rows: List[Dict[str, Any]] = []
    fit_rows: List[Dict[str, Any]] = []
    grid_rows: List[Dict[str, Any]] = []
    needed = int(timing_cfg.timing_max_paths_per_model) + int(timing_cfg.timing_warmup_paths_per_model)
    counts: Dict[str, int] = {m: 0 for m in active_models}

    def _enough() -> bool:
        return all(counts[m] >= needed for m in active_models)
    ...
    for day_i, rep_day in enumerate(rep_days):
        ...
        for b_center in bucket_centers:
            ...
            expiries = calls_b.groupby("expiration")["tau"].first().sort_values()
            if expiries.empty:
                continue
            contract_exp = pd.Timestamp(expiries.index[int(np.argmin(np.abs(expiries.values * 365.0 - b_center)))])
            anchor_slice = calls_b[calls_b["expiration"] == contract_exp].copy()
            if anchor_slice.empty:
                continue

            F0 = float(anchor_slice["F"].iloc[0])
            q_raw = float(anchor_slice["q_hat_raw"].iloc[0])
            q = float(anchor_slice["q_hat"].iloc[0])
            r = float(anchor_slice["r"].iloc[0])
            T_mkt = float(anchor_slice["tau"].iloc[0])
            S0_anchor = float(anchor_slice["S"].iloc[0])
            ...
            # per-slice fit timing
            sigma_bs = float("nan")
            _maybe_cuda_sync(); t0 = time.perf_counter()
            if cfg.run_bs:
                sigma_bs = fit_sigma_bucket(cal_df)
            _maybe_cuda_sync(); t1 = time.perf_counter()
            if cfg.run_bs:
                fit_rows.append({
                    "symbol": cfg.symbol, "date": pd.Timestamp(rep_day), "bucket": b_label,
                    "model": "BS", "timing_kind": "slice_fit", "seconds": float(t1 - t0),
                    "n_surface_rows": int(len(cal_df)),
                })
            ...
            if cfg.run_jd:
                fit_rows.append({
                    "symbol": cfg.symbol, "date": pd.Timestamp(rep_day), "bucket": b_label,
                    "model": "JD", "timing_kind": "slice_fit", "seconds": float(t1 - t0),
                    "n_surface_rows": int(len(cal_df)),
                })
            ...
            if cfg.run_heston:
                fit_rows.append({
                    "symbol": cfg.symbol, "date": pd.Timestamp(rep_day), "bucket": b_label,
                    "model": "SV", "timing_kind": "slice_fit", "seconds": float(t1 - t0),
                    "n_surface_rows": int(len(cal_df)),
                })
            ...
            if cfg.run_qlbs:
                ...
                fit_rows.append({
                    "symbol": cfg.symbol, "date": pd.Timestamp(rep_day), "bucket": b_label,
                    "model": "QLBS", "timing_kind": "slice_fit", "seconds": float(t1 - t0),
                    "n_surface_rows": int(len(cal_df)), "n_epochs": 2000,
                })
            ...
            if cfg.run_rlop:
                ...
                fit_rows.append({
                    "symbol": cfg.symbol, "date": pd.Timestamp(rep_day), "bucket": b_label,
                    "model": "RLOP", "timing_kind": "slice_fit", "seconds": float(t1 - t0),
                    "n_surface_rows": int(len(cal_df)), "n_epochs": 2000,
                })
            ...
            if cfg.world == "gbm":
                ...
                seed_db = int(cfg.seed + day_i * 10007 + b_center * 101)
                S_paths = simulate_spot_paths(
                    S0=S0_anchor, r=r, q=q, sigma_true=sigma_true, T=T_used,
                    n_steps=n_steps_run, n_paths=int(cfg.n_paths), seed=seed_db,
                )
                T = T_used
            elif cfg.world == "historical":
                S_paths, dt_years, path_dates, world_status, maturity_date_used, expiry_gap_days = build_historical_path(
            ...
            if use_grid:
                ...
                for opt_idx in range(len(selected)):
                    ...
                    for model in price_fns.keys():
                        ...
                        grid_rows.append({
                            "symbol": cfg.symbol, "date": pd.Timestamp(rep_day), "bucket": b_label,
                            "model": model, "timing_kind": "delta_grid_build", "seconds": float(t1 - t0),
                            "opt_idx": int(opt_idx), "K": float(K0), "n_steps_run": int(S_paths.shape[1] - 1),
                            "grid_n": int(cfg.grid_n),
                        })
            ...
            for opt_idx in range(len(selected)):
                ...
                for model in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    is_warmup = counts[model] < int(timing_cfg.timing_warmup_paths_per_model)
                    counts[model] += 1
                    path_rows.append({
                        "symbol": cfg.symbol,
                        "date": pd.Timestamp(rep_day),
                        ...
                        "timed_index_for_model": int(counts[model]),
                        "n_steps_run": int(S_paths.shape[1] - 1),
                        "hist_delta_method": str(cfg.hist_delta_method),
                        "used_grid": bool(use_grid),
                    })
    ...
    path_df_keep = path_df.loc[~path_df["is_warmup"].astype(bool)].copy() if not path_df.empty else path_df
    path_summary = _summarize_timing_rows(path_df_keep, "seconds", ["model"])
    if not path_summary.empty:
        meta = path_df_keep.groupby("model", dropna=False).agg(
            n_steps_mean=("n_steps_run", "mean"),
            n_steps_median=("n_steps_run", "median"),
            used_grid=("used_grid", "max"),
        ).reset_index()
        path_summary = path_summary.merge(meta, on="model", how="left")

    fit_summary = _summarize_timing_rows(fit_df, "seconds", ["model"]) if not fit_df.empty else pd.DataFrame()
    grid_summary = _summarize_timing_rows(grid_df, "seconds", ["model"]) if not grid_df.empty else pd.DataFrame()

    if outp is not None:
        if timing_cfg.save_path_rows and not path_df.empty:
            path_df.to_csv(outp / "timing_path_rows.csv", index=False)
        if timing_cfg.save_fit_rows and not fit_df.empty:
            fit_df.to_csv(outp / "timing_fit_rows.csv", index=False)
        if timing_cfg.save_grid_rows and not grid_df.empty:
            grid_df.to_csv(outp / "timing_grid_rows.csv", index=False)
        path_summary.to_csv(outp / "timing_path_summary.csv", index=False)
        if not fit_summary.empty:
            fit_summary.to_csv(outp / "timing_fit_summary.csv", index=False)
        if not grid_summary.empty:
            grid_summary.to_csv(outp / "timing_grid_summary.csv", index=False)
...
def main_cli() -> None:
    ap = argparse.ArgumentParser(description="Historical hedging metrics + lightweight efficiency timing for BS/JD/SV/QLBS/RLOP.")
    ap.add_argument("--input_csv", type=str, required=True, help="Path to preprocessed calls csv (same schema used by the paper runs).")
    ap.add_argument("--symbol", type=str, required=True, choices=["SPY", "XOP"])
    ap.add_argument("--start_date", type=str, required=True)
    ap.add_argument("--end_date", type=str, required=True)
    ap.add_argument("--buckets", type=str, default="28", help="Comma-separated bucket centers in days.")
    ap.add_argument("--moneyness_targets", type=str, default="1.00,1.03", help="Comma-separated targets, e.g. 1.00,1.03")
    ap.add_argument("--world", type=str, default="historical", choices=["historical", "gbm"])
    ap.add_argument("--rep_day_mode", type=str, default="all", choices=["all", "even", "middle"])
    ap.add_argument("--n_rep_days", type=int, default=90)
    ap.add_argument("--hist_mode", type=str, default="to_expiry", choices=["to_expiry", "fixed_steps"])
    ap.add_argument("--hist_fixed_steps", type=int, default=28)
    ap.add_argument("--hist_max_expiry_gap_days", type=int, default=1)
    ap.add_argument("--hist_delta_method", type=str, default="local_fd", choices=["local_fd", "grid"], help="Paper-faithful timing is local_fd. grid is a deployment-oriented speed variant.")
    ap.add_argument("--friction", type=float, default=4e-3)
    ap.add_argument("--grid_n", type=int, default=61)
    ap.add_argument("--grid_n_std", type=float, default=4.0)
    ap.add_argument("--heston_u_max", type=float, default=60.0)
    ap.add_argument("--heston_n_points", type=int, default=201)
    ap.add_argument("--qlbs_risk_lambda", type=float, default=0.01)
    ap.add_argument("--rlop_risk_lambda", type=float, default=0.10)
    ap.add_argument("--qlbs_checkpoint_tmpl", type=str, default="trained_model/test8/risk_lambda={rl:.1e}/policy_1.pt")
    ap.add_argument("--rlop_checkpoint", type=str, default="trained_model/testr9/policy_1.pt")
    ap.add_argument("--timing_max_paths_per_model", type=int, default=50)
    ap.add_argument("--timing_warmup_paths_per_model", type=int, default=5)
    ap.add_argument("--out_dir", type=str, default="")
    ap.add_argument("--run_tag", type=str, default="")
    ap.add_argument("--disable_bs", action="store_true")
    ap.add_argument("--disable_jd", action="store_true")
    ap.add_argument("--disable_sv", action="store_true")
    ap.add_argument("--disable_qlbs", action="store_true")
    ap.add_argument("--disable_rlop", action="store_true")
    args = ap.parse_args()
    ...
    cfg = HedgingRunConfig(
        ...
        buckets=tuple(int(x) for x in str(args.buckets).split(",") if str(x).strip()),
        n_rep_days=int(args.n_rep_days),
        rep_day_mode=str(args.rep_day_mode),
        moneyness_targets=tuple(float(x) for x in str(args.moneyness_targets).split(",") if str(x).strip()),
        world=str(args.world),
        friction=float(args.friction),
        hist_mode=str(args.hist_mode),
        hist_fixed_steps=int(args.hist_fixed_steps),
        hist_max_expiry_gap_days=int(args.hist_max_expiry_gap_days),
        hist_delta_method=str(args.hist_delta_method),
        run_bs=(not bool(args.disable_bs)),
        run_jd=(not bool(args.disable_jd)),
        run_heston=(not bool(args.disable_sv)),
        run_qlbs=(not bool(args.disable_qlbs)),
        run_rlop=(not bool(args.disable_rlop)),
        grid_n=int(args.grid_n),
        grid_n_std=float(args.grid_n_std),
        heston_u_max=float(args.heston_u_max),
        heston_n_points=int(args.heston_n_points),
        qlbs_risk_lambda=float(args.qlbs_risk_lambda),
        rlop_risk_lambda=float(args.rlop_risk_lambda),
        qlbs_checkpoint_tmpl=str(args.qlbs_checkpoint_tmpl),
        rlop_checkpoint=str(args.rlop_checkpoint),
        out_dir=(str(args.out_dir) if str(args.out_dir).strip() else None),
        run_tag=(str(args.run_tag) if str(args.run_tag).strip() else None),
    )
    timing_cfg = TimingRunConfig(
        timing_max_paths_per_model=int(args.timing_max_paths_per_model),
        timing_warmup_paths_per_model=int(args.timing_warmup_paths_per_model),
    )
    df_all = _read_csv_auto(str(args.input_csv))
    out = benchmark_efficiency_spy_like(df_all=df_all, cfg=cfg, timing_cfg=timing_cfg)

    print("\n[efficiency] path summary (steady-state; warmup excluded):")
    if out["path_summary"].empty:
        print("<empty>")
    else:
        print(out["path_summary"].to_string(index=False))

    print("\n[efficiency] slice-fit summary:")
    if out["fit_summary"].empty:
        print("<empty>")
    else:
        print(out["fit_summary"].to_string(index=False))

    if not out["grid_summary"].empty:
        print("\n[efficiency] delta-grid build summary:")
        print(out["grid_summary"].to_string(index=False))
