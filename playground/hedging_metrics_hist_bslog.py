...
def append_bs_delta_log_rows(
    ...
    """Append one row per BS-delta evaluation to a CSV log.

    The first six columns are exactly the BS-delta inputs: S, K, tau, r, q, sigma.
    We also store delta_bs plus optional context columns for auditability.
    """
    if log_path is None:
        return
    ...
    if S_arr.size != delta_arr.size:
        raise ValueError(f"S size {S_arr.size} != delta size {delta_arr.size}")

    rows: List[Dict[str, Any]] = []
    base_ctx = dict(context or {})
    for i, (s_i, d_i) in enumerate(zip(S_arr, delta_arr)):
        row: Dict[str, Any] = {
            "S": float(s_i),
            "K": float(K),
            "tau": float(tau),
        ...
        row.update(base_ctx)
        row["path_index"] = int(i)
        rows.append(row)

    log_path.parent.mkdir(parents=True, exist_ok=True)
    df_rows = pd.DataFrame(rows)
    write_header = not log_path.exists()
    df_rows.to_csv(log_path, mode="a", header=write_header, index=False)
...
class HedgingRunConfig:
    ...
    bs_delta_log_path: Optional[str] = None
...
def run_dynamic_hedging_spy_like(df_all: pd.DataFrame, cfg: HedgingRunConfig) -> pd.DataFrame:
    ...
    bs_delta_log_path: Optional[Path] = None
    if cfg.bs_delta_log_path:
        candidate = Path(cfg.bs_delta_log_path)
        if not candidate.is_absolute() and outp is not None:
            candidate = outp / candidate
        bs_delta_log_path = candidate
        bs_delta_log_path.parent.mkdir(parents=True, exist_ok=True)
                ...
                for model in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    if model == "BS":
                        def get_delta(step_idx: int, S_vec: np.ndarray) -> np.ndarray:
                            ...
                            delta_bs = bs_delta_from_spot(S_vec, K0, tau, r, q, float(sigma_bs))
                            append_bs_delta_log_rows(
                                log_path=bs_delta_log_path,
                                S=S_vec,
                                K=K0,
                                tau=tau,
                                r=r,
                                q=q,
                                sigma=float(sigma_bs),
                                delta=delta_bs,
                                context={
                                    "symbol": cfg.symbol,
                                    "rep_day": pd.Timestamp(rep_day).date().isoformat(),
                                    "bucket": b_label,
                                    "contract_expiration": pd.Timestamp(contract_exp).date().isoformat(),
...
def run_bs_historical_from_csv(
    data_csv: str,
    symbol: str,
    start_date: str,
    end_date: str,
    ...
    out_dir: str,
    run_tag: Optional[str] = None,
    rep_day_mode: str = "all",
    n_rep_days: int = 90,
    friction: float = 4e-3,
    hist_mode: str = "to_expiry",
    hist_fixed_steps: int = 28,
    hist_max_expiry_gap_days: int = 1,
) -> pd.DataFrame:
    ...
    cfg = HedgingRunConfig(
        ...
        seed=123,
        hist_mode=hist_mode,
        hist_fixed_steps=hist_fixed_steps,
        hist_max_expiry_gap_days=hist_max_expiry_gap_days,
        hist_delta_method="local_fd",
        run_bs=True,
        run_jd=False,
        run_heston=False,
        ...
        out_dir=out_dir,
        run_tag=run_tag,
        bs_delta_log_path="bs_delta_inputs.csv",
    )
    return run_dynamic_hedging_spy_like(df_all=df, cfg=cfg)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run BS-only historical hedging and log BS delta inputs.")
    parser.add_argument("--data-csv", required=True, help="Path to preprocessed calls CSV.")
    parser.add_argument("--symbol", required=True, help="Ticker symbol, e.g. SPY or XOP.")
    parser.add_argument("--start-date", required=True, help="Inclusive start date, YYYY-MM-DD.")
    parser.add_argument("--end-date", required=True, help="Inclusive end date, YYYY-MM-DD.")
    parser.add_argument("--buckets", nargs="+", type=int, required=True, help="Maturity bucket centers in days, e.g. 14 28 56.")
    parser.add_argument("--moneyness-targets", nargs="+", type=float, required=True, help="Moneyness targets in K/F, e.g. 1.0 1.03.")
    parser.add_argument("--out-dir", required=True, help="Directory under which the run folder will be created.")
    parser.add_argument("--run-tag", default=None, help="Optional run subfolder name.")
    parser.add_argument("--rep-day-mode", choices=["even", "middle", "all"], default="all")
    parser.add_argument("--n-rep-days", type=int, default=90)
    parser.add_argument("--friction", type=float, default=4e-3)
    parser.add_argument("--hist-mode", choices=["to_expiry", "fixed_steps"], default="to_expiry")
    parser.add_argument("--hist-fixed-steps", type=int, default=28)
    parser.add_argument("--hist-max-expiry-gap-days", type=int, default=1)
    args = parser.parse_args()

    hedge_res = run_bs_historical_from_csv(
        data_csv=args.data_csv,
        symbol=args.symbol,
        start_date=args.start_date,
        end_date=args.end_date,
        buckets=tuple(args.buckets),
        moneyness_targets=tuple(args.moneyness_targets),
        out_dir=args.out_dir,
        run_tag=args.run_tag,
        rep_day_mode=args.rep_day_mode,
        n_rep_days=args.n_rep_days,
        friction=args.friction,
        hist_mode=args.hist_mode,
        hist_fixed_steps=args.hist_fixed_steps,
        hist_max_expiry_gap_days=args.hist_max_expiry_gap_days,
    )

    print(f"Saved hedge results to: {Path(args.out_dir) / (args.run_tag or '')}")
    print("Rows produced:", len(hedge_res))
    show_cols = [c for c in ["date", "bucket", "moneyness_target", "model", "error_T", "cost_total", "shortfall_prob"] if c in hedge_res.columns]
    if show_cols:
        print(hedge_res[show_cols].head(10).to_string(index=False))
