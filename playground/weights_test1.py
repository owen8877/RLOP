...
def b76_vega(F: float, K: float, tau: float, r: float, sigma: float) -> float:
    """
    Black-76 vega for a call:
      C = DF * (F * Phi(d1) - K * Phi(d2))
      vega = ∂C/∂sigma = DF * F * phi(d1) * sqrt(tau)
    We approximate DF = exp(-r * tau), same as b76_price_call.
    """
    ...
    d1 = (math.log(F / K) + 0.5 * sigma * sigma * tau) / v
    DF = math.exp(-r * tau)
    return DF * F * _norm_pdf(d1) * sqrt_tau


def make_qlbs_weights_economic(
    g: pd.DataFrame,
    alpha: float = 5.0,
    ...
    """
    Build per-option weights for QLBS/RLOP calibration on one bucket cross-section g.
    Uses only contract features (F, K, tau, r, sigma_mkt_b76, C_mid), not model errors.

    w = w_vega * w_moneyness * w_price, then normalized to mean 1.
    """
    F = g["F"].to_numpy(dtype=float)
    K = g["strike"].to_numpy(dtype=float)
    tau = g["tau"].to_numpy(dtype=float)
    r = g["r"].to_numpy(dtype=float)
    sigma = g["sigma_mkt_b76"].to_numpy(dtype=float)
    C = g["C_mid"].to_numpy(dtype=float)

    # 1) vega-based weights (approximate IV weighting)
    vegas = np.array(
        [b76_vega(F[i], K[i], tau[i], r[i], sigma[i]) for i in range(len(g))],
        dtype=float,
    )
    w_vega = 1.0 / np.maximum(vegas, vega_floor) ** 2

    # 2) Gaussian in log-moneyness: emphasize near-ATM
    m = np.log(K / F)  # log-moneyness based on forward
    w_mny = np.exp(-alpha * m ** 2)

    # 3) Price-based smoothness: down-weight very cheap options
    w_price = C / (C + lambda_price)

    # 4) Combine and normalize to mean 1
    w = w_vega * w_mny * w_price
    w_mean = w.mean()
    ...
    for day in iterator:
        ...
        for bucket, g in calls.groupby("bucket"):
            ...
            if run_qlbs:
                ...
                # Build economic weights for this (day, bucket) cross-section g
                weights = make_qlbs_weights_economic(
                    g,
                    alpha=5.0,          # how strongly to focus near-ATM
                    lambda_price=2.0,   # how hard to down-weight cheap options
                    vega_floor=1.0,     # vega clamp
                )
