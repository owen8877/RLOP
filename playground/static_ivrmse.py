...
def b76_iv_from_price(
    ...
    t = max(float(target), float(intrinsic))
...
# ============================================================
# Put–call parity: infer F and DF (for "european" path)
# ============================================================
...
# ============================================================
# Maturity bucketing & BS sigma fit
# ============================================================
...
def _heston_cf(
    ...
    """Heston CF for log-forward (little Heston trap). Vectorized over u."""
    x = math.log(F)
    iu = 1j * u
...
def heston_price_call(
    ...
    """Heston call price under forward measure via P1/P2."""
    DF = math.exp(-r * tau)
...
def prepare_calls_one_day_symbol(
    ...
    # ---- FAST-PATH for 'american' (calls-only) ----
    if type.lower() == "american":
        df = df[df["cp"].astype(str).str.upper().eq("CALL")]
        ...
        hdr = df.groupby(["date", "symbol", "expiration"], as_index=False).agg(F=("F", "first"), DF=("DF", "first"))
        ...
        calls = calls.merge(hdr[["date", "symbol", "expiration", "F", "DF", "r"]], on=["date", "symbol", "expiration"], how="left")
    ...
    # ---- 'european' path: Calls–Puts pairing + parity inference ----
    pvt = df.pivot_table(
...
def _pooled_rmse_x1000(block: pd.DataFrame, iv_col: str, weight_col: str = "N") -> float:
    ...
    `iv_col` is IVRMSE×1000 column.
    """
    x = block[iv_col].to_numpy(dtype=float)
...
def _bucket_days(bucket_label: str) -> int:
    m = re.match(r"^\s*(\d+)\s*d\s*$", str(bucket_label))
    if not m:
        raise ValueError(f"Unrecognized bucket label: {bucket_label!r}")
...
def preprocess_deribit(
    ...
    *,
    ...
    """
    Minimal mapper for Deribit options for the 'european' path.
    Outputs: ['date','act_symbol','expiration','strike','cp','bid','ask','mid']
    """
    def _parse(name: str):
...
# ============================================================
# Main entry: summarize symbol over a period (IVRMSE only)
# ============================================================
...
def summarize_symbol_period_ivrmse(
    ...
    """
    Summarize IVRMSE for a symbol over a period (per bucket):
      • Primary  = equal-day mean IVRMSE (each day counts once)
      • Secondary= pooled/N-weighted IVRMSE (every contract counts)
    Saves: daily table, equal_day_mean, pooled, daily log, and (optional) per-option predicted prices.
    """
    df_pre = adapter_eur_calls_to_summarizer(df_all) if type == "american" else preprocess_deribit(df_all)
    ...
    if out_dir is not None:
        ...
        (outp / "run_config.txt").write_text(
            ...
            f"run_qlbs={run_qlbs} run_rlop={run_rlop}\n",
            encoding="utf-8",
        )
    ...
    for day in iterator:
        ...
        for bucket, g in calls.groupby("bucket"):
            ...
            bucket_days = _bucket_days(bucket)
            anchor_T = bucket_days / 252.0  # IMPORTANT: per-bucket anchor_T
            ...
            # Heston (SV)
            if run_heston:
                h_params, _ = calibrate_heston_bucket(g, u_max=100.0, n_points=501)
            ...
            if run_qlbs:
                ...
                Qmodel = QLBSModel(
                    ...
                    anchor_T=anchor_T,  # IMPORTANT: bucket/252
                )

                spot = float(g["F"].iloc[0])
                ...
                try:
                    ...
                    def qlbs_price(F, K, tau, r):
                        ...
                        return float(_res.estimated_prices[0])
    ...
    # Save per-option predicted prices (wide)
    if outp is not None and len(preds_rows) > 0:
        ...
        preds_wide = preds_all.pivot_table(index=keys, columns="model", values="price_hat", aggfunc="first").reset_index()

        rename_map = {"BS": "price_BS", "JD": "price_JD", "SV": "price_SV", "QLBS": "price_QLBS", "RLOP": "price_RLOP"}
        for k, v in rename_map.items():
            if k in preds_wide.columns:
                preds_wide = preds_wide.rename(columns={k: v})
...
def make_publication_table(
    ...
    """
    Build a paper-style table:
      Sections: Whole sample / Moneyness <1 / >1 / >1.03
      Rows: one per bucket like 'SPY (τ=14d)'
      Columns: BS, JD, SV (Heston), optionally QLBS/RLOP if present.
...
def main_spy20():
    ...
    res = summarize_symbol_period_ivrmse(
        ...
        show_progress=True,
        print_daily=True,
        out_dir="SPY_20Q1_ivrmse_clean",
    )
    make_publication_table(res, symbol="SPY", measure="equal", buckets=[14, 28, 56], decimals=2, out_dir="SPY_20Q1_ivrmse_clean")
    #make_publication_table(res, symbol="SPY", measure="pooled", buckets=[14, 28, 56], decimals=2, out_dir="SPY_20Q1_ivrmse_clean")
...
def main_spy25():
    ...
    res = summarize_symbol_period_ivrmse(
        ...
        show_progress=True,
        print_daily=True,
        out_dir="SPY_25Q2_ivrmse_clean",
    )
    make_publication_table(res, symbol="SPY", measure="equal", buckets=[14, 28, 56], decimals=2, out_dir="SPY_25Q2_ivrmse_clean")
    #make_publication_table(res, symbol="SPY", measure="pooled", buckets=[14, 28, 56], decimals=2, out_dir="SPY_25Q2_ivrmse_clean")
...
def main_xop20():
    ...
    res = summarize_symbol_period_ivrmse(
        ...
        show_progress=True,
        print_daily=True,
        out_dir="XOP_20Q1_ivrmse_clean",
    )
    make_publication_table(res, symbol="XOP", measure="equal", buckets=[14, 28, 56], decimals=2, out_dir="XOP_20Q1_ivrmse_clean")
    #make_publication_table(res, symbol="XOP", measure="pooled", buckets=[14, 28, 56], decimals=2, out_dir="XOP_20Q1_ivrmse_clean")
...
def main_xop25():
    ...
    res = summarize_symbol_period_ivrmse(
        ...
        show_progress=True,
        print_daily=True,
        out_dir="XOP_25Q2_ivrmse_clean",
    )
    make_publication_table(res, symbol="XOP", measure="equal", buckets=[14, 28, 56], decimals=2, out_dir="XOP_25Q2_ivrmse_clean")
    #make_publication_table(res, symbol="XOP", measure="pooled", buckets=[14, 28, 56], decimals=2, out_dir="XOP_25Q2_ivrmse_clean")
...
def main_btc():
    ...
    res = summarize_symbol_period_ivrmse(
        ...
        show_progress=True,
        print_daily=True,
        out_dir="BTC_09NOV25_ivrmse_clean",
    )
    make_publication_table(res, symbol="BTC", measure="equal", buckets=[7, 30, 90], decimals=2, out_dir="BTC_09NOV25_ivrmse_clean")
    #make_publication_table(res, symbol="BTC", measure="pooled", buckets=[7, 30, 90], decimals=2, out_dir="BTC_09NOV25_ivrmse_clean")
