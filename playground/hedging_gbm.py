...
def prepare_calls_one_day_symbol(
    ...
    IMPORTANT:
      - computes BOTH q_hat_raw and q_hat (clipped); never overwrites raw.

    Returns calls surface with:
      date,symbol,expiration,tau,strike,C_mid,F,DF,r,S,PV_div,
      q_hat_raw, q_hat,
...
def calibrate_heston_bucket(calls_bucket: pd.DataFrame, u_max: float = 60.0, n_points: int = 201) -> Tuple[Dict[str, float], float]:
    ...
    def sse_vec(p: np.ndarray) -> float:
        ...
        phat = np.array(
            [heston_price_call(F[i], K[i], tau[i], r[i], params, u_max=u_max, n_points=n_points) for i in range(len(C))],
            dtype=float,
        )
        err = phat - C
        return float(np.dot(err, err))
...
def precompute_delta_grid(
    ...
    for t in range(n_steps):
        tau = max(T - t * dt, 0.0)
        prices[t, :] = np.array(
            [price_scalar_fn(float(Sg[i]), float(K), float(tau), float(r), float(q)) for i in range(grid_n)],
            dtype=float,
        )
...
def hedge_short_call_paths(
    ...
    """
    Delta hedge a SHORT call on a dividend-paying underlying (continuous yield q).

    Self-financing with dividends:
      - cash earns r
      - stock position delta earns dividend flow q * S * delta dt (credited to cash)
    """
    ...
    for t in range(1, n_steps):
        # accrue interest on cash over (t-1, t)
        cash *= math.exp(r * dt)

        # receive dividends on shares held over (t-1, t)
        if q != 0.0:
            S_prev = S_paths[:, t - 1]
...
class HedgingRunConfig:
    ...
    # q clipping (used only for safety if a caller passes raw q)
    q_clip_lo: float = -0.10
    q_clip_hi: float = 0.20
...
def run_dynamic_hedging_spy_like(
    ...
    for day_i, rep_day in enumerate(rep_days):
        ...
        for b_center in bucket_centers:
            ...
            # NOTE: q_hat_raw is truly raw; q_hat is the clipped value produced by prepare_calls_one_day_symbol.
            ...
            # Final safety clamp (should be a no-op if q_hat exists)
            ...
            for opt_idx in range(len(selected)):
                ...
                for model in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    row = {
                        ...
                        "F0": float(F0),
                        "r": float(r),
                        "q_hat_raw": float(q_raw),  # truly raw
                        "q_hat": float(q),          # clipped value actually used everywhere
...
def make_hedging_publication_table(
    ...
    """
    GBM world publication table aligned with IVRMSE layout.
    ...
    if metric not in {"RMSE_hedge", "avg_cost", "shortfall_prob", "shortfall_1pct", "cvar_1pct"}:
...
def hedging_spy20():
    ...
    out_dir = "SPY_20Q1_hedging_v_gbm"
    ...
    print("\nDynamic hedging summary (SPY 20Q1, GBM world):")
    summary = (
        hedge_res
        ...
        .agg(
            ...
            shortfall1pct_mean=("shortfall_1pct", "mean"),
            cvar1pct_mean=("cvar_1pct", "mean"),
            N=("RMSE_hedge", "size"),
        )
...
def hedging_xop20():
    ...
    out_dir = "XOP_20Q1_hedging_v_gbm"
    ...
    print("\nDynamic hedging summary (XOP 20Q1, GBM world):")
    summary = (
        hedge_res
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
...
def hedging_spy25():
    ...
    out_dir = "SPY_25Q2_hedging_v_gbm"
    ...
    print("\nDynamic hedging summary (SPY 25Q2, GBM world):")
    summary = (
        hedge_res
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
...
def hedging_xop25():
    ...
    out_dir = "XOP_25Q2_hedging_v_gbm"
    ...
    print("\nDynamic hedging summary (XOP 25Q2, GBM world):")
    summary = (
        hedge_res
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
