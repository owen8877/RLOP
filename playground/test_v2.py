...
def run_dynamic_hedging_for_symbol_period(
    ...
    """
    Step-2 corrected dynamic hedging runner (Spot/Forward consistent):

    - Uses spot S0 (from preprocessed data if available; else infers S0 ≈ F*DF + PV_div).
    - Infers dividend yield q_hat from F, S, r, tau (continuous-yield approximation).
    - Simulates spot paths under drift (r - q_hat).
    - Prices models using forward F_t = S_t * exp((r - q_hat) * tau_remaining).
    - Delta is computed w.r.t spot via finite differences of price(S).

    Still uses your fairness fix:
      premium0 = market mid at t=0 (same across models).
    """
    # --- Preprocess calls (same entry logic as IVRMSE) ---
    if type == "american":
        df_pre = adapter_eur_calls_to_summarizer(df_all)
    else:
        df_pre = preprocess_deribit(df_all)
    ...
    # Representative day (keep your current behavior)
    ...
    # Import RL models once (if needed)
    if run_qlbs:
        from lib.qlbs2.test_trained_model import QLBSModel
    if run_rlop:
        from lib.rlop2.test_trained_model import RLOPModel
    ...
    for dlab in bucket_labels:
        calls_b = calls[calls["bucket"] == dlab].copy()
        if calls_b.empty:
            print(f"[dynamic hedging] bucket {dlab} has no contracts on {rep_day.date()}, skipping.")
            continue

        # World vol = median market IV in this bucket
        ...
        r = float(row_atm["r"])
        F0 = float(row_atm["F"])
        DF0 = float(row_atm["DF"])

        # Spot + dividend yield inference (continuous-yield approximation)
        PV_div0 = float(row_atm["PV_div"]) if "PV_div" in row_atm.index and pd.notna(row_atm["PV_div"]) else 0.0

        if "S" in row_atm.index and pd.notna(row_atm["S"]) and float(row_atm["S"]) > 0:
            S0 = float(row_atm["S"])
        else:
            # from your conversion identity: F = (S - PV_div)/DF  -> S = F*DF + PV_div
            S0 = float(F0 * DF0 + PV_div0)

        # if q_hat already computed upstream, use it; else infer
        if "q_hat" in row_atm.index and pd.notna(row_atm["q_hat"]):
            q_hat = float(row_atm["q_hat"])
        else:
            q_hat = r - math.log(max(F0, 1e-12) / max(S0, 1e-12)) / max(T, 1e-12)

        # Premium at t=0 (market, shared across models)
        premium0 = float(row_atm["C_mid"])

        # ------------------------------
        # Calibrations on the bucket
        # ------------------------------
        ...
        # RL fits: keep the “underlying” variable consistent with your pricing representation (forward)
        ...
        if run_qlbs:
            ...
            q_result = Qmodel.fit(
                spot=F0,  # IMPORTANT: forward representation
                time_to_expiries=time_to_expiries,
                strikes=strikes,
                r=r,
        ...
        # ----------------------------------------
        # Spot->Forward mapping for pricing at time t
        # ----------------------------------------
        def _F_from_spot(S_vec: np.ndarray, tau_rem: float) -> np.ndarray:
            S_vec = np.asarray(S_vec, dtype=float)
            if tau_rem <= 0:
                return S_vec.copy()
            return S_vec * np.exp((r - q_hat) * tau_rem)

        # ------------------------------
        # Price fns as functions of SPOT
        # ------------------------------
        price_fns = {}
        delta_fns = {}
        ...
        if run_bs and np.isfinite(sigma_bs):
            def bs_price_fn(S_vec, K_, tau_, r_):
                F_vec = _F_from_spot(np.atleast_1d(S_vec), tau_)
                out = np.empty_like(F_vec, dtype=float)
                for i, Fi in enumerate(F_vec):
                    out[i] = b76_price_call(float(Fi), K_, tau_, r_, float(sigma_bs))
                return out
        ...
        if run_jd and jd_params:
            ...
            def jd_price_fn(S_vec, K_, tau_, r_):
                ...
                out = np.empty_like(F_vec, dtype=float)
                for i, Fi in enumerate(F_vec):
                    out[i] = merton_price_call_b76(float(Fi), K_, tau_, r_, sigma_jd, lam_jd, muJ_jd, dJ_jd)
                return out
        ...
        if run_heston and h_params:
            def sv_price_fn(S_vec, K_, tau_, r_):
                ...
                out = np.empty_like(F_vec, dtype=float)
                for i, Fi in enumerate(F_vec):
                    out[i] = heston_price_call(float(Fi), K_, tau_, r_, h_params, u_max=60.0, n_points=201)
                return out
        ...
        # ------------------------------
        # Simulate SPOT world with q_hat
        # ------------------------------
