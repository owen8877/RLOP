...
def prepare_calls_one_day_symbol(
    ...
    hdr["q_hat"] = hdr["q_hat_raw"].clip(q_clip_lo, q_clip_hi)
...
# ============================================================
# Delta engines
#   - Grid-based: best for many paths (GBM)
#   - Local FD: best for a single realized path (Historical)
# ============================================================
...
def precompute_delta_grid(
    ...
    r: float,
    q: float,
    tau_grid: np.ndarray,   # length n_steps: remaining tau at each hedge time
    S_grid: np.ndarray,
) -> np.ndarray:
...
# ============================================================
# Hedging engine (supports variable dt via dt_years)
# ============================================================
...
def hedge_short_call_paths(
    ...
    Delta hedge a SHORT call with:
      - cash accrues at r
      - shares receive dividend yield q
...
def build_spot_history_from_df(df_pre: pd.DataFrame, symbol: str) -> pd.Series:
    """
    Build a daily spot series from df_pre (quotes table).
    Preference:
      1) median of provided S
      2) fallback median of (DF*F + PV_div) if S missing
    Returns: pd.Series indexed by date (normalized), values S (>0)
    """
...
def build_historical_path(
    ...
    *,
    mode: str,
    fixed_steps: int,
    max_end_gap_days: int,
) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[List[pd.Timestamp]], str]:
    """
    Returns:
      S_path: (1, n_steps+1)
      dt_years: (n_steps,)
      dates: list length n_steps+1
      status: "ok" or reason for skip

    mode:
      - "to_expiry": use trading dates from start..end (must have enough coverage near end)
      - "fixed_steps": use start + next fixed_steps trading days
    """
    if spot.empty:
        return None, None, None, "no_spot_history"
    ...
    if start_date not in all_dates:
        ...
        if pos >= len(all_dates):
            return None, None, None, "start_date_after_spot_range"
    ...
    if mode == "fixed_steps":
        ...
        if end_pos >= len(all_dates):
            return None, None, None, "insufficient_future_spot_for_fixed_steps"
        dates = list(all_dates[pos:end_pos + 1])
        # end_date is informational only here
        end_used = dates[-1]
        gap_days = int((pd.Timestamp(end_date) - pd.Timestamp(end_used)).days)
        # we don't enforce gap for fixed_steps
    elif mode == "to_expiry":
        # take all trading dates in [start_date, end_date]
        sub = spot.loc[(spot.index >= start_date) & (spot.index <= end_date)]
        if sub.empty or len(sub) < 2:
            return None, None, None, "insufficient_spot_between_start_and_end"
        dates = list(sub.index)

        end_used = dates[-1]
        gap_days = int((pd.Timestamp(end_date) - pd.Timestamp(end_used)).days)
        if gap_days > int(max_end_gap_days):
            return None, None, None, f"end_gap_too_large(gap_days={gap_days})"
    else:
        return None, None, None, f"unknown_hist_mode({mode})"

    Svec = spot.loc[dates].to_numpy(dtype=float)
    if not np.all(np.isfinite(Svec)) or np.any(Svec <= 0):
        return None, None, None, "nonfinite_or_nonpositive_spot_in_path"

    # dt in years using calendar day deltas
    ...
    if np.any(dt_years <= 0):
        return None, None, None, "nonpositive_dt_in_hist_path"

    S_path = Svec.reshape(1, -1)
    return S_path, dt_years, [pd.Timestamp(x).normalize() for x in dates], "ok"
...
class HedgingRunConfig:
    ...
    hist_fixed_steps: int = 28         # used if hist_mode="fixed_steps"
    hist_max_end_gap_days: int = 1     # for to_expiry: require end_used within this many days of expiry
    ...
    # Delta grid controls (major speed lever for GBM)
    grid_n: int = 61
...
def run_dynamic_hedging_spy_like(df_all: pd.DataFrame, cfg: HedgingRunConfig) -> pd.DataFrame:
    ...
    for day_i, rep_day in enumerate(rep_days):
        ...
        for b_center in bucket_centers:
            ...
            q = float(anchor_row["q_hat"]) if "q_hat" in anchor_row.index else float(np.clip(q_raw, cfg.q_clip_lo, cfg.q_clip_hi))
            ...
            if cfg.world == "gbm":
                # keep your original: constant dt, fixed n_steps, MC paths
                T = float(T_mkt)
                n_steps_run = int(cfg.n_steps)
                tau_grid = np.array([max(T - (T / n_steps_run) * t, 0.0) for t in range(n_steps_run)], dtype=float)
                ...
                dt_years = None  # constant dt inferred by hedger
                path_dates = None
                path_end = None
                end_gap_days = None

            elif cfg.world == "historical":
                # build realized path from rep_day to anchor_exp (or fixed steps)
                S_paths, dt_years, path_dates, world_status = build_historical_path(
                    spot=spot_hist,
                    start_date=pd.Timestamp(rep_day).normalize(),
                    end_date=anchor_exp,
                    mode=str(cfg.hist_mode),
                    fixed_steps=int(cfg.hist_fixed_steps),
                    max_end_gap_days=int(cfg.hist_max_end_gap_days),
                )
                if world_status != "ok" or S_paths is None or dt_years is None or path_dates is None:
                    # still record skip info as rows? We'll just skip producing hedge rows, but note in console via result row.
                    results.append({
                        "symbol": cfg.symbol,
                ...
                # tau at each hedge time = remaining to anchor_exp
                tau_grid = np.array([(anchor_exp - pd.Timestamp(path_dates[t])).days / 365.0 for t in range(n_steps_run)], dtype=float)
                tau_grid = np.clip(tau_grid, 0.0, None)

                path_end = pd.Timestamp(path_dates[-1]).normalize()
                end_gap_days = int((anchor_exp - path_end).days)

                # redefine T as realized sum(dt_years) for cash accrual consistency
                T = float(dt_years.sum())
            ...
            # All non-BS models use B76 under the hood via F(S,tau)
            # --------------------------------------------------------
            ...
            if use_grid:
                # reference sigma_ref: sigma_true (for range), time horizon T
                S_grid = build_S_grid(S0=S0, sigma_ref=sigma_true, T=T, grid_n=cfg.grid_n, n_std=cfg.grid_n_std)
            ...
            for opt_idx in range(len(selected)):
                ...
                premium0_world_bs = bs_call_from_spot(S0, K0, float(T_mkt), r, q, sigma_true)
                ...
                for model in ["BS", "JD", "SV", "QLBS", "RLOP"]:
                    ...
                    if cfg.world == "historical":
                        row.update({
                            ...
                            "path_end_date": pd.Timestamp(path_dates[-1]).isoformat() if path_dates else None,
                            "path_end_gap_days": int(end_gap_days) if end_gap_days is not None else None,
            ...
            if outp is not None:
                ...
                cal_df_min = cal_df[
                    ["date","symbol","expiration","tau","strike","C_mid","F","DF","r","S","PV_div","q_hat_raw","q_hat","sigma_mkt_b76","moneyness_F","moneyness_S"]
                ].copy()
                cal_df_min.to_csv(subdir / "calibration_surface.csv", index=False)

                sel_min = selected[
                    ["expiration","tau","strike","C_mid","F","S","r","q_hat_raw","q_hat","sigma_mkt_b76","moneyness_target","moneyness_actual_F","moneyness_actual_S"]
                ].copy()
                ...
                calib = {
                    ...
                    "hist": {
                        ...
                        "hist_max_end_gap_days": int(cfg.hist_max_end_gap_days),
                    },
...
# ============================================================
# Publication table (pooled RMSE via err2_mean)
# ============================================================
...
def make_hedging_publication_table(
    ...
    for sec_name, sec_key in sections:
        for b in bucket_labels:
            ...
            sub = hedge_res[(hedge_res["bucket"] == b) & _bin_mask(hedge_res, sec_key)].copy()

            for m in models:
                v = sub.loc[sub["model"] == m]
                if v.empty:
                    row[m] = np.nan
                    continue

                if metric == "RMSE_hedge" and pooled_rmse and "err2_mean" in v.columns:
                    row[m] = float(np.sqrt(np.mean(v["err2_mean"].astype(float))))
                else:
                    row[m] = float(np.mean(v[metric].astype(float)))
...
def hedging_spy20(world: str = "gbm"):
    ...
    print(f"\nDynamic hedging summary (SPY 20Q1, world={world}):")
    summary = (
        hedge_res[hedge_res["model"].notna()]
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
            RMSE_pool=("err2_mean", lambda x: float(np.sqrt(np.mean(x.astype(float))))) if "err2_mean" in hedge_res.columns else ("RMSE_hedge", "mean"),
            cost_mean=("avg_cost", "mean"),
            shortfall_mean=("shortfall_prob", "mean"),
            N=("RMSE_hedge", "size"),
        )
        .sort_values(["bucket", "moneyness_target", "RMSE_pool"])
    )
    ...
    for met in ["RMSE_hedge", "avg_cost", "shortfall_prob", "shortfall_1pct", "cvar_1pct"]:
        ...
        print(f"\nPublication table ({met}, world={world}):")
...
def hedging_xop20(world: str = "gbm"):
    ...
    print(f"\nDynamic hedging summary (XOP 20Q1, world={world}):")
    summary = (
        hedge_res[hedge_res["model"].notna()]
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
...
def hedging_spy25(world: str = "gbm"):
    df = pd.read_csv("data/spy_preprocessed_calls_25q2.csv")
    ...
    print(f"\nDynamic hedging summary (SPY 25Q2, world={world}):")
    summary = (
        hedge_res[hedge_res["model"].notna()]
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
...
def hedging_xop25(world: str = "gbm"):
    df = pd.read_csv("data/xop_preprocessed_calls_25q2.csv")
    ...
    print(f"\nDynamic hedging summary (XOP 25Q2, world={world}):")
    summary = (
        hedge_res[hedge_res["model"].notna()]
        .groupby(["bucket", "moneyness_target", "model"], as_index=False)
        .agg(
...
class Test(TestCase):
    def test_main(self):
        ...
        # Uncomment if your 25Q2 datasets include enough SPOT coverage through expiries
