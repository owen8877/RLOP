...
class Experiment2(TestCase):
    ...
    def _path(self, simplified, risk_lambda, mutation_lambda, exp_name=exp_name):
    ...
    def test_bs_value(self):
        ...
        for risk_lambda in risk_lambdas:
            ...
            collector = policy_gradient(env, bs_policy, nn_baseline, episode_n=4000, pi_frozen=True, plot=False)
    ...
    def test_draw_plot(self):
        ...
        for exp in [1, 2]:
            baselines[exp] = dict()
            for risk_lambda in risk_lambdas:
                path = self._path(simplified, risk_lambda, mutation_lambda, exp_name=exp_name.replace('2', str(exp)))
                baseline = NNBaseline(simplified=simplified, alpha=0, from_filename=f'{path}/baseline.pt')
